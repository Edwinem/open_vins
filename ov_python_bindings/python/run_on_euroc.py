...
from openvins_config import get_options_from_launch_file,get_openvins_options_from_yaml
import os
...
@click.option('--config_file', required=True,
              help="Path to a config file from OpenVINS(Most likely a yaml file found in open_vins/config). Contains the settings for the algorithm")
@click.option('--use_viewer', is_flag=True, default=True, help="Use a 3D viwer to view the camera path")
def run_on_euroc(euroc_folder, start_timestamp, config_file, use_viewer):
    """
    Runs OpenVINS on the euroc dataset.

    Example:
        This start timestamp is for MH_02.

        python run_on_euroc --euroc_folder <PATH_TO_EUROC>/mav0 --start_timestamp 1403636859536666624 --config_file
        <PATH_TO_OPENVINS>/open_vins/config/euroc_mav/estimator_config.yaml


    """

    options = get_openvins_options_from_yaml(config_file)
