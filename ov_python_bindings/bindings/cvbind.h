...
PYBIND11_TYPE_CASTER(cv::KeyPoint, _("KeyPoint"));
  ...
  bool load(handle src, bool) {
    if (!src) return false;
    pybind11::tuple pt = reinterpret_borrow<pybind11::tuple>(src.attr("pt"));

    value=cv::KeyPoint(pt[0].cast<float>(), pt[1].cast<float>(), src.attr("size").cast<float>()
        ,src.attr("angle").cast<float>(), src.attr("response").cast<float>(), src.attr("octave").cast<int>(),src.attr("class_id").cast<int>());

    return true;
  }

  static handle cast(cv::KeyPoint keypoint, return_value_policy /*policy*/, handle /*parent*/) {
    return pybind11::make_tuple(keypoint.pt.x, keypoint.pt.y, keypoint.size, keypoint.angle, keypoint.response, keypoint.octave,keypoint.class_id).release();
  }
};
...
//  {
//
//    cv::KeyPoint k = reinterpret_borrow<cv::KeyPoint>(obj);
//    value=k;
//    return true;
//  }
//
//  static handle cast(const cv::KeyPoint& k , return_value_policy, handle)
//  {
//    return handle();
//  }
//};
